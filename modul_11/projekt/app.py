...
#klasa db z podstawowymi operacjami na plikach zródłowych
class db:
    def __init__(self):
    ...
    #sprawdzanie i konwertowanie formatu dat na d/m/y
    @staticmethod
...
#łączenie bazy traksakcji z odpowiednimi kat. produktów, płcią klienta, krajemn sprzedaży
    def merge(self):
...
#inicjacja aplikacji i wykorzystanie zewnętrznych arkuszy stylów
...
#tworzenie layoutu - dodanie widgeta z zakładkami
app.layout = html.Div([html.Div([dcc.Tabs(id='tabs',value='tab-1',children=[ #tworzenie zakładek umożliwiających nawigacje po dashboardzie; wartości 1 i 2, gdzie 1  to docelowy widok
                            dcc.Tab(label='Sprzedaż globalna',value='tab-1'), #div z zakładkami "sprzedaż globalna" i "produkty"
                            dcc.Tab(label='Produkty',value='tab-2'),
                            dcc.Tab(label='Kanały sprzedaży', value='tab-3')
                            ]),
                            html.Div(id='tabs-content')
                    ],style={'width':'80%','margin':'auto'})], #div z docelowym widgetem (w=80%, margin=auto)
                    style={'height':'100%'})
...
###tab3 callbacks
@app.callback(
    Output('Store-type-sales-avg', 'figure'),
    [Input('Store-type-dropdown', 'value')]
)

def update_avg_sales(Store_type):
    filtered_df = df.merged[df.merged['Store_type'] == Store_type]
    avg_sales = filtered_df.groupby('day_of_week')['total_amt'].mean()

    trace = go.Bar(
        x=avg_sales.index,
        y=avg_sales.values,
        name=f'Avg Sales ({Store_type})',
        hoverinfo='x+y'
    )

    fig = go.Figure(data=[trace])
    fig.update_layout(
        title=f'Średnia sprzedaż dla kanału {Store_type}',
        xaxis_title="Dzień tygodnia",
        yaxis_title="Średnia sprzedaż",
    )
